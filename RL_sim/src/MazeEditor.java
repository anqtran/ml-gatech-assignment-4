...
//	private JComboBox jComboBox1;
	...
	private void initGUI() {
		try {
			...
			myMaze = new Maze(45,45);
					for (int i = 0; i < 45 ; i++) {
			for (int j = 0; j < 45 ; j++) {
				double present = Math.random();
				if (present  > 0.2) {
					int dir = (int) Math.floor(Math.random() * 4);
					myMaze.addWall(new Wall(i,j,dir,50));
				}
...
//				System.out.println("nodes= "+nodeX+","+nodeY);
				...
				{
					case 1:
						int lowerLimX = (nodeX*nodeLength) + (int)(0.5*nodeLength);
						int upperLimX = ((nodeX+1)*nodeLength) - (int)(0.5*nodeLength);
						if(clickX<lowerLimX)
						{
